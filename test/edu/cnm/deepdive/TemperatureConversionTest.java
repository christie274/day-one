...
class TemperatureConversionTest {
  ...
  void convertC2FScalar() {
    for (int i = 0; i < celsiusTemperatures.length; i++) {
      assertEquals(fahrenheitTemperatures[i],TemperatureConversion.convertC2F(celsiusTemperatures[i]), TOLERANCE);
    }
  ...
  void convertC2FArray() {
    assertArrayEquals(fahrenheitTemperatures, TemperatureConversion.convertC2F(celsiusTemperatures), TOLERANCE);
  }

  @Test
  void convertF2CArray() {
    assertArrayEquals(celsiusTemperatures, TemperatureConversion.convertF2C(fahrenheitTemperatures), TOLERANCE);
  }
}